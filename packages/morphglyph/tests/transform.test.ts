...
describe('Transform with the shipped Japanese font', () => {
  it.each([
    ...
    ['', ''],
    ['A A', 'AAA'],
  ])('preserves endpoints and finite geometry: %s → %s', (a, b) => {
    const from = text(a),
      to = text(b),
      plan = createPlan(from, to);
    ...
    for (const pathArc of [0, Math.PI / 3, -Math.PI, 2 * Math.PI]) {
      ...
      for (const progress of [0.1, 0.25, 0.5, 0.75, 0.9]) {
        ...
        if (current.glyphs.length) {
          ...
          expect(bounds.minX).toBeGreaterThanOrEqual(box.minX - 0.0001);
          expect(bounds.maxX).toBeLessThanOrEqual(box.maxX + 0.0001);
          expect(bounds.minY).toBeGreaterThanOrEqual(box.minY - 0.0001);
          expect(bounds.maxY).toBeLessThanOrEqual(box.maxY + 0.0001);
        }
      }
    }
  });
  ...
  it('keeps holes in the endpoint glyphs and is deterministic', () => {
    ...
    expect(frame(a, 0.5)).toEqual(frame(b, 0.5));
  });
  ...
  it('can continue from an intermediate frame without jumping', () => {
    const middle = frame(createPlan(text('ABC'), text('XYZ')), 0.4);
...
it('splits a Bezier without changing the curve', () => {
  ...
  for (const t of [0, 0.2, 0.5, 0.7, 1]) {
    const p = pointOn(curve, t);
    const q = t <= 0.5 ? pointOn(left, t * 2) : pointOn(right, t * 2 - 1);
...
it('uses a circle through both points and has a stable linear limit', () => {
  const a = { x: 0, y: 0 },
    b = { x: 2, y: 0 };
  expect(arcPoint(a, b, 0.5, Math.PI).x).toBeCloseTo(1);
  expect(arcPoint(a, b, 0.5, Math.PI).y).toBeCloseTo(-1);
  expect(arcPoint(a, b, 0.5, 1e-9)).toEqual({ x: 1, y: 0 });
  expect(arcPoint(a, b, 0.5, NaN)).toEqual({ x: 1, y: 0 });
});
