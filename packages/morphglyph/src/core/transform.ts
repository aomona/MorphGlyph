import { alignStart, area, boundsOf, center, collapse, contains, reverse, sample, subdivide, union } from './geometry';
import { clamp, mix, mixPoint, type Contour, type ContourPair, type Cubic, type Easing, type Glyph, type MorphPlan, type Point, type Quality, type Shape } from './types';

type Outline = { contour: Contour; center: Point; area: number; depth: number; parent: number; original: number };
function topology(contours: Contour[]): Outline[] {
  ...
  return contours.map((contour, i) => {
    const containers = polygons.map((p, j) => ({ p, j })).filter(({ p, j }) => sizes[j] > sizes[i] + 1e-6 && contains(p, polygons[i][0]));
    const parent = containers.sort((a, b) => sizes[a.j] - sizes[b.j])[0]?.j ?? -1;
    const depth = containers.length;
    const desiredSign = depth % 2 === 0 ? 1 : -1;
    return { contour: Math.sign(area(polygons[i])) === desiredSign ? contour : reverse(contour), center: center(polygons[i]), area: sizes[i], depth, parent, original: i };
  }).sort((a, b) => a.depth - b.depth || b.area - a.area);
}

function pairContours(a: Glyph, b: Glyph, quality: Quality): ContourPair[] {
  const from = topology(a.contours), to = topology(b.contours);
  const used = new Set<number>();
  const parents = new Map<number, number>();
  const pairs: ContourPair[] = [];
  const ac = center(from.map(c => c.center)), bc = center(to.map(c => c.center));
  ...
  for (const source of from) {
    ...
    to.forEach((target, j) => {
      ...
      const score = Math.hypot(source.center.x - ac.x - target.center.x + bc.x, source.center.y - ac.y - target.center.y + bc.y)
        + Math.abs(Math.sqrt(source.area) - Math.sqrt(target.area));
      if (score < cost) { cost = score; best = j; }
    });
    if (best === -1) {
      ...
      const start = subdivide(source.contour, count), end = subdivide(to[best].contour, count);
      pairs.push({ from: start, to: alignStart(start, end) });
    }
  }
  to.forEach((target, i) => {
    if (!used.has(i)) pairs.push({ from: collapse(target.contour, target.center), to: target.contour });
  });
  return pairs;
}
...
export function createPlan(from: Shape, to: Shape, quality: Quality = 'balanced'): MorphPlan {
  const pairs = Array.from({ length: Math.max(from.glyphs.length, to.glyphs.length) }, (_, i) => ({
    contours: pairContours(from.glyphs[i] ?? { contours: [] }, to.glyphs[i] ?? { contours: [] }, quality),
  }));
...
export function ease(t: number, easing: Easing = 'smooth'): number {
  ...
  return clamp((sigmoid(10 * (t - .5)) - error) / (1 - 2 * error));
}
...
export function arcPoint(a: Point, b: Point, t: number, angle: number): Point {
  ...
  const cos = Math.cos(angle * t), sin = Math.sin(angle * t);
  return { x: cx + (a.x - cx) * cos - (a.y - cy) * sin, y: cy + (a.x - cx) * sin + (a.y - cy) * cos };
}

export function frame(plan: MorphPlan, progress: number, options: { easing?: Easing; pathArc?: number; stagger?: number; duration?: number } = {}): Shape {
  const p = clamp(progress);
  if (p === 0) return plan.from;
  if (p === 1) return plan.to;
  const duration = Math.max(0, options.duration ?? 1000);
  const gap = Math.min(Math.max(0, options.stagger ?? 0), duration * .9 / Math.max(1, plan.pairs.length - 1));
  ...
  const glyphs = plan.pairs.map((pair, i) => {
    ...
    return { contours: pair.contours.map(c => c.from.map((s, j) => s.map((v, k) => arcPoint(v, c.to[j][k], t, options.pathArc ?? 0)) as Cubic)) };
  });
...
export function trajectoryBounds(plan: MorphPlan, angle: number) {
  ...
  // Sampled displacement plus a conservative arc-length bound between samples.
  for (const pair of plan.pairs) for (const c of pair.contours) for (let i = 0; i < c.from.length; i++) for (let k = 0; k < 4; k++) {
    const a = c.from[i][k], b = c.to[i][k];
    const radius = Math.hypot(b.x - a.x, b.y - a.y) / (2 * Math.abs(Math.sin(angle / 2)));
    const margin = radius * Math.abs(angle) / 64;
    for (let t = 0; t <= 32; t++) {
      const p = arcPoint(a, b, t / 32, angle);
      bounds.minX = Math.min(bounds.minX, p.x - margin); bounds.maxX = Math.max(bounds.maxX, p.x + margin);
      bounds.minY = Math.min(bounds.minY, p.y - margin); bounds.maxY = Math.max(bounds.maxY, p.y + margin);
    }
  }
  return bounds;
}
