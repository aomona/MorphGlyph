...
export class MorphGlyphError extends Error {
  constructor(public readonly code: 'FONT_LOAD' | 'MISSING_GLYPH' | 'TEXT_LIMIT' | 'GEOMETRY', message: string, options?: ErrorOptions) {
...
export function loadFont(source?: FontSource): Promise<FontHandle> {
  ...
  const url = source ?? new URL('../assets/NotoSansJP-Regular.otf', import.meta.url).href;
  let pending = fonts.get(url);
  if (!pending) {
    pending = fetch(url).then(async response => {
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      return parseFont(await response.arrayBuffer(), url);
    }).catch((cause: unknown) => {
      fonts.delete(url);
...
export function layoutText(handle: FontHandle, text: string, size: number, spacing: number, align: 'left' | 'center' | 'right'): Shape {
  text = text.replace(/\r\n?|\n/g, ' ').normalize('NFC');
  if (Array.from(text).length > 256) throw new MorphGlyphError('TEXT_LIMIT', 'MorphGlyph supports up to 256 code points per string.');
  const key = JSON.stringify([text, size, spacing, align]);
  let cache = shapes.get(handle);
  if (!cache) { cache = new Map(); shapes.set(handle, cache); }
  const cached = cache.get(key);
  if (cached) return cached;
  const font = handle.font;
  const glyphs = font.stringToGlyphs(text);
  if (glyphs.some(g => g.index === 0)) throw new MorphGlyphError('MISSING_GLYPH', 'This font does not contain every requested glyph.');
  const scale = size / font.unitsPerEm;
  let x = 0;
  const result = glyphs.map((g, i) => {
    const contours = contoursFromCommands(g.getPath(x, 0, size).commands);
    if (contours.reduce((n, c) => n + c.length, 0) > 4096) throw new MorphGlyphError('GEOMETRY', 'A glyph exceeds the geometry safety limit.');
  ...
  const shifted = result.map(g => ({ contours: g.contours.map(c => c.map(s => s.map(p => ({ x: p.x + offset, y: p.y })) as Cubic)) }));
  const ink = boundsOf(shifted);
  const shape: Shape = {
    glyphs: shifted, advance: x,
    bounds: { minX: Math.min(ink.minX, offset), maxX: Math.max(ink.maxX, offset + x), minY: Math.min(ink.minY, -font.ascender * scale), maxY: Math.max(ink.maxY, -font.descender * scale) },
  };
  cache.set(key, shape);
