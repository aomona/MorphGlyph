...
export function Surface({ plan, options, reduced, snapshot, controls, semantic }: {
  ...
  useLayoutEffect(() => {
    ...
    let started = false, completed = false, disposed = false, raf = 0;
    ...
    const draw = () => {
      ...
      const display = r ? (actual < .5 ? 0 : 1) : actual;
      const shape = frame(plan, display, { easing: o.easing, pathArc: o.pathArc, stagger: o.stagger, duration: o.duration });
      snapshot.current = shape;
      while (paths.length < shape.glyphs.length) {
        const path = document.createElementNS('http://www.w3.org/2000/svg', 'path');
        path.setAttribute('fill-rule', 'nonzero');
        container.appendChild(path); paths.push(path);
      }
    ...
    const schedule = () => {
      if (!disposed && !raf && running && !completed && document.visibilityState !== 'hidden' && latest.current.options.progress === undefined) raf = requestAnimationFrame(tick);
    };
    const tick = (now: number) => {
      raf = 0;
      if (disposed) return;
      const { options: o, reduced: r } = latest.current;
      if (o.progress !== undefined || !running || o.playing === false) { previous = 0; return; }
      let elapsed = previous ? now - previous : 0;
      previous = now;
      if (wait > 0) { const consumed = Math.min(wait, elapsed); wait -= consumed; elapsed -= consumed; }
      if (wait > 0) { schedule(); return; }
      if (!started) { started = true; o.onStart?.(); }
      const duration = Math.max(0, o.duration ?? 1000);
      position = r || duration === 0 ? (sign > 0 ? 1 : 0) : clamp(position + sign * elapsed / duration);
      draw();
    ...
    controls.current = {
      play() {
        if (latest.current.options.progress !== undefined || latest.current.options.playing === false) return;
        running = true; schedule();
      },
      pause() {
        if (latest.current.options.progress !== undefined || latest.current.options.playing === true) return;
        running = false; stop();
      },
      restart() {
        if (latest.current.options.progress !== undefined) return;
        stop(); sign = latest.current.options.direction === 'reverse' ? -1 : 1;
        position = sign > 0 ? 0 : 1; completed = false; started = false;
    ...
    sync.current = () => {
      ...
      if (o.progress !== undefined) { stop(); draw(); return; }
      if (o.playing !== undefined) running = o.playing;
      if (!running) stop();
      draw(); schedule();
    };
    const visibilityChanged = () => { stop(); if (document.visibilityState !== 'hidden') schedule(); };
    document.addEventListener('visibilitychange', visibilityChanged);
    draw(); schedule();
    return () => { disposed = true; stop(); document.removeEventListener('visibilitychange', visibilityChanged); container.replaceChildren(); controls.current = idleControls(); sync.current = noop; };
  }, [plan, controls, snapshot]);

  useEffect(() => { sync.current(); }, [options.progress, options.playing, options.easing, options.pathArc, options.duration, options.stagger, reduced]);
  useEffect(() => { controls.current.restart(); }, [options.direction, controls]);

  return <svg aria-hidden="true" focusable="false" data-morphglyph-svg="" viewBox={`${box.minX - pad} ${box.minY - pad} ${width} ${height}`} width={options.width ?? width} height={options.height ?? height} style={{ display: 'block', maxWidth: '100%', overflow: 'visible' }} fill="currentColor">
    <g ref={group} />
  </svg>;
}
